...
capped at ~2.1 billion. `pgq upgrade` widens the existing columns (and their
legacy sequences) to `BIGINT` automatically.

!!! warning "This migration takes an `ACCESS EXCLUSIVE` lock"
    On a large `queue` table the rewrite can stall the queue for seconds to
    minutes. Run `pgq upgrade` during a maintenance window, or widen the table
    out-of-band first. See
    [Widening `id` to BIGINT](../guides/bigint-id-migration.md) for the full
    explanation, the locking details, and a zero-downtime procedure.
