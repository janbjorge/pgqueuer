...
now use `BIGINT GENERATED BY DEFAULT AS IDENTITY`, and `pgq upgrade` widens
existing `int4` columns in place:
...
!!! warning "This migration takes an `ACCESS EXCLUSIVE` lock"
    ...
    which blocks **everything** on the table — enqueues, dequeues, and plain
    `SELECT`s. The rewrite time grows with row count, so on a large or bloated
    `queue` table this can stall the queue for seconds to minutes. While the
    statement *waits* to acquire the lock, new queries queue up behind it, so a
