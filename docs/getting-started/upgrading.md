...
### `id` columns widened to `BIGINT` identity (#671)
...
now use `BIGINT GENERATED BY DEFAULT AS IDENTITY`; `pgq upgrade` widens the
existing `int4` columns to `BIGINT` and swaps their legacy `SERIAL` sequences
for identity sequences, so an upgraded schema is identical to a fresh one. The
conversion preserves the id number line (the identity restarts past the old
sequence, never reusing an id). The migration is idempotent (guarded on the
column type and identity attribute) and safe to re-run.
