...
`pgqueuer_statistics.time_in_queue` was `INTERVAL NOT NULL` and has not been
written since v0.19. On a database that old, every statistics insert failed on
the not-null constraint, and the unique index still keyed on the column, so
aggregation could not match its `ON CONFLICT` target either.

`pgq upgrade` fixes both: it rebuilds the index and drops the `NOT NULL`. It
does **not** drop the column -- that is data, and discarding it is your call. It
prints a note instead, on every run, until you do:
