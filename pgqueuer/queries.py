...
class Queries:
    ...
    async def install(self) -> None:
        ...
        Uses the migration framework to install all schema objects. This ensures
        that install and upgrade use the same code path, preventing divergence.
        ...
        migration_list = list(migrations.create_migrations_list(self.qbe.settings))
    ...
    async def upgrade(self) -> None:
        ...
        Uses the new migration framework to track and apply only pending migrations.
        This ensures migrations are only run once and provides better tracking
        of schema changes over time.

        For databases previously upgraded with the old system, this method will
        automatically detect and seed the migration history on first run.
    ...
    async def _seed_migration_history(self) -> None:
        """
        Seed the migration history table for databases upgraded with the old system.

        For databases that already have all the schema changes applied via the old
        upgrade system, this method checks which columns/tables exist and marks
        the corresponding migrations as applied.
        ...
        # Check if we already have migration history
        applied = await manager.get_applied_migrations()
        if applied:
            # Migration history exists, no need to seed
            return

        # Check if this is a legacy database by looking for key schema elements
        # If the database has the queue_table_log table, it's likely been upgraded
        has_log_table = await self.has_table(self.qbe.settings.queue_table_log)
        ...
        if has_log_table:
            ...
            for migration in migration_list:
                # Record each migration without executing it
                await manager.record_migration(migration)

            print(f"✓ Seeded {len(migration_list)} migrations in history table")
