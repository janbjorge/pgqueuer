...
class TimedOverflowBuffer(Generic[T]):
    ...
    _tasks: set[asyncio.Task] = dataclasses.field(
        init=False,
        default_factory=set,
    )
    ...
    async def __aenter__(self) -> Self:
        self._add_task(asyncio.create_task(self._periodic_flush()))
        return self
        ...
        for _ in range(5):
            ...
            try:
                ...
                logconfig.logger.warning(
                    "Shutdown flush failed, %d items remaining", len(items)
                )
                for item in items:
                    self.events.put_nowait(item)
    ...
    def _schedule_flush(self) -> None:
        self._add_task(asyncio.create_task(self.flush()))
