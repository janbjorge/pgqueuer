...
class QueryQueueBuilder:
    ...
    def build_mark_jobs_as_failed_query(self) -> str:
        """
        Generate SQL query to mark jobs as failed instead of moving to log table.
        
        This allows jobs to remain in the queue table with 'failed' status
        for manual intervention instead of being automatically moved to the log.
        
        Returns:
            str: The SQL query string to mark jobs as failed.
        """
        return f"""
        UPDATE {self.settings.queue_table} 
    ...
    def build_requeue_failed_jobs_query(self) -> str:
        """
        Generate SQL query to re-queue failed jobs.
        
        Changes the status of failed jobs back to 'queued' and resets
        their execute_after time to now for immediate processing.
        
        Returns:
            str: The SQL query string to re-queue failed jobs.
        """
        return f"""
        UPDATE {self.settings.queue_table}
        SET 
    ...
    def build_list_failed_jobs_query(self) -> str:
        """
        Generate SQL query to list all failed jobs.
        
        Returns:
            str: The SQL query string to list failed jobs.
        """
        return f"""
        SELECT * FROM {self.settings.queue_table}
        WHERE status = 'failed'
