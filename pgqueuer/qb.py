...
class QueryBuilderEnvironment:
    ...
    $$ LANGUAGE plpgsql;"""
        ...
        yield f"""CREATE UNLOGGED TABLE {self.settings.queue_table_log} (
        id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        created TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL,
        job_id BIGINT NOT NULL,
        status TEXT NOT NULL,
...
class QueryQueueBuilder:
    ...
    updated AS (
        ...
        INSERT INTO {self.settings.queue_table_log} (
            ...
            status,
            entrypoint
        ) SELECT id, status, entrypoint FROM updated
    )
    ...
    def create_enqueue_query(self) -> str:
        ...
        WITH inserted AS (
            ...
            RETURNING id, entrypoint, status
        )
        INSERT INTO {self.settings.queue_table_log}
        (job_id, status, entrypoint)
        SELECT id, 'queued', entrypoint
        FROM inserted
    ...
    def create_delete_from_queue_query(self) -> str:
        ...
        return f"""WITH deleted AS (
            ...
            RETURNING id, entrypoint
        )
        INSERT INTO {self.settings.queue_table_log} (job_id, status, entrypoint)
        SELECT id, 'delete' AS status, entrypoint FROM deleted
        """
    ...
    def create_log_job_query(self) -> str:
        ...
        SELECT id, status, entrypoint FROM merged
        """
