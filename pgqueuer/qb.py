...
class QueryBuilderEnvironment:
    ...
    def build_install_query(self) -> str:
        ...
        return f"""CREATE {durability_policy.queue_table} TABLE {self.settings.queue_table} (
    ...
    ALTER TABLE {self.settings.queue_table} ADD CONSTRAINT {self.settings.queue_table}_status_check CHECK (status IN ('queued', 'picked', 'successful', 'exception', 'canceled', 'deleted'));
    ...
    CREATE INDEX {self.settings.queue_table}_heartbeat_id_id1_idx ON {self.settings.queue_table} (heartbeat ASC, id DESC) INCLUDE (id)
        WHERE status = 'picked';
    ...
    ALTER TABLE {self.settings.queue_table_log} ADD CONSTRAINT {self.settings.queue_table_log}_status_check CHECK (status IN ('queued', 'picked', 'successful', 'exception', 'canceled', 'deleted'));
    ...
    ALTER TABLE {self.settings.statistics_table} ADD CONSTRAINT {self.settings.statistics_table}_status_check CHECK (status IN ('queued', 'picked', 'successful', 'exception', 'canceled', 'deleted'));
    ...
    ALTER TABLE {self.settings.schedules_table} ADD CONSTRAINT {self.settings.schedules_table}_status_check CHECK (status IN ('queued', 'picked', 'successful', 'exception', 'canceled', 'deleted'));
    ...
    $$ LANGUAGE plpgsql;"""
        ...
        yield f"ALTER TABLE {self.settings.statistics_table} ALTER COLUMN status TYPE TEXT;"  # noqa
        ...
        yield f"""DROP INDEX IF EXISTS {self.settings.queue_table}_priority_id_id1_idx;"""  # noqa
        yield f"""DROP INDEX IF EXISTS {self.settings.queue_table}_updated_id_id1_idx;"""  # noqa
        yield f"""DROP INDEX IF EXISTS {self.settings.queue_table}_heartbeat_id_id1_idx;"""  # noqa
        yield f"""ALTER TABLE {self.settings.queue_table} ALTER COLUMN status TYPE TEXT;"""  # noqa
        yield f"""ALTER TABLE {self.settings.queue_table} ADD CONSTRAINT {self.settings.queue_table}_status_check CHECK (status IN ('queued', 'picked', 'successful', 'exception', 'canceled', 'deleted'));"""  # noqa
        yield f"""CREATE INDEX {self.settings.queue_table}_priority_id_id1_idx ON {self.settings.queue_table} (priority ASC, id DESC) INCLUDE (id) WHERE status = 'queued';"""  # noqa
        yield f"""CREATE INDEX {self.settings.queue_table}_updated_id_id1_idx ON {self.settings.queue_table} (updated ASC, id DESC) INCLUDE (id) WHERE status = 'picked';"""  # noqa
        yield f"""CREATE INDEX {self.settings.queue_table}_heartbeat_id_id1_idx ON {self.settings.queue_table} (heartbeat ASC, id DESC) INCLUDE (id) WHERE status = 'picked';"""  # noqa

        yield f"""DROP INDEX IF EXISTS {self.settings.queue_table_log}_status;"""  # noqa
        yield f"""ALTER TABLE {self.settings.queue_table_log} ALTER COLUMN status TYPE TEXT;"""  # noqa
        yield f"""ALTER TABLE {self.settings.queue_table_log} ADD CONSTRAINT {self.settings.queue_table_log}_status_check CHECK (status IN ('queued', 'picked', 'successful', 'exception', 'canceled', 'deleted'));"""  # noqa
        yield f"""CREATE INDEX {self.settings.queue_table_log}_status ON {self.settings.queue_table_log} (status);"""  # noqa

        yield f"""DROP INDEX {self.settings.statistics_table}_unique_count;"""  # noqa
        yield f"""ALTER TABLE {self.settings.statistics_table} DROP COLUMN IF EXISTS time_in_queue ;"""  # noqa
        yield f"""CREATE UNIQUE INDEX {self.settings.statistics_table}_unique_count ON {self.settings.statistics_table} (priority, DATE_TRUNC('sec', created at time zone 'UTC'), (status::TEXT), entrypoint);"""  # noqa
        yield f"""ALTER TABLE {self.settings.statistics_table} ALTER COLUMN status TYPE TEXT;"""  # noqa
        yield f"""ALTER TABLE {self.settings.statistics_table} ADD CONSTRAINT {self.settings.statistics_table}_status_check CHECK (status IN ('queued', 'picked', 'successful', 'exception', 'canceled', 'deleted'));"""  # noqa

        yield f"""ALTER TABLE {self.settings.schedules_table} ALTER COLUMN status DROP DEFAULT;"""  # noqa
        yield f"""ALTER TABLE {self.settings.schedules_table} ALTER COLUMN status TYPE TEXT;"""  # noqa
        yield f"""ALTER TABLE {self.settings.schedules_table} ADD CONSTRAINT {self.settings.schedules_table}_status_check CHECK (status IN ('queued', 'picked', 'successful', 'exception', 'canceled', 'deleted'));"""  # noqa
...
class QueryQueueBuilder:
    ...
    def build_delete_from_queue_query(self) -> str:
        ...
        SELECT id, 'deleted' AS status, entrypoint, priority
        FROM deleted
        """
