...
def create_migrations_list() -> list[Migration]:
    ...
    migrations = [
        Migration(
            ...
            sql_generator=lambda: f"""CREATE TYPE {settings.queue_status_type} AS ENUM ('queued', 'picked', 'successful', 'exception', 'canceled', 'deleted');""",
        ),
        Migration(
            version="002",
            description="Create pgqueuer queue table",
            sql_generator=lambda: f"""CREATE {durability.queue_table} TABLE {settings.queue_table} (
        ...
        Migration(
            ...
            sql_generator=lambda: f"""CREATE INDEX {settings.queue_table}_priority_id_id1_idx ON {settings.queue_table} (priority ASC, id DESC) INCLUDE (id) WHERE status = 'queued';""",
        ),
        Migration(
            version="004",
            description="Create index on updated for picked jobs",
            sql_generator=lambda: f"""CREATE INDEX {settings.queue_table}_updated_id_id1_idx ON {settings.queue_table} (updated ASC, id DESC) INCLUDE (id) WHERE status = 'picked';""",
        ),
        Migration(
            version="005",
            description="Create index on queue_manager_id",
            sql_generator=lambda: f"""CREATE INDEX {settings.queue_table}_queue_manager_id_idx ON {settings.queue_table} (queue_manager_id) WHERE queue_manager_id IS NOT NULL;""",
        ),
        Migration(
            version="006",
            description="Create unique index on dedupe_key",
            sql_generator=lambda: f"""CREATE UNIQUE INDEX {settings.queue_table}_unique_dedupe_key ON {settings.queue_table} (dedupe_key) WHERE ((status IN ('queued', 'picked') AND dedupe_key IS NOT NULL));""",
        ),
        Migration(
            version="007",
            description="Create pgqueuer_log table",
            sql_generator=lambda: f"""CREATE {durability.queue_log_table} TABLE {settings.queue_table_log} (id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, created TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL, job_id BIGINT NOT NULL, status {settings.queue_status_type} NOT NULL, priority INT NOT NULL, entrypoint TEXT NOT NULL, traceback JSONB DEFAULT NULL, aggregated BOOLEAN DEFAULT FALSE);""",
        ),
        Migration(
            version="008",
            description="Create index on pgqueuer_log for not aggregated",
            sql_generator=lambda: f"""CREATE INDEX {settings.queue_table_log}_not_aggregated ON {settings.queue_table_log} ((1)) WHERE not aggregated;""",
        ),
        Migration(
            version="009",
            description="Create index on pgqueuer_log created column",
            sql_generator=lambda: f"""CREATE INDEX {settings.queue_table_log}_created ON {settings.queue_table_log} (created);""",
        ),
        Migration(
            version="010",
            description="Create index on pgqueuer_log status column",
            sql_generator=lambda: f"""CREATE INDEX {settings.queue_table_log}_status ON {settings.queue_table_log} (status);""",
        ),
        Migration(
            version="011",
            description="Create index on pgqueuer_log job_id and created",
            sql_generator=lambda: f"""CREATE INDEX {settings.queue_table_log}_job_id_status ON {settings.queue_table_log} (job_id, created DESC);""",
        ),
        Migration(
            version="012",
            description="Create pgqueuer_statistics table",
            sql_generator=lambda: f"""CREATE {durability.statistics_table} TABLE {settings.statistics_table} (id SERIAL PRIMARY KEY, created TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT DATE_TRUNC('sec', NOW() at time zone 'UTC'), count BIGINT NOT NULL, priority INT NOT NULL, status {settings.queue_status_type} NOT NULL, entrypoint TEXT NOT NULL);""",
        ),
        Migration(
            version="013",
            description="Create unique index on pgqueuer_statistics",
            sql_generator=lambda: f"""CREATE UNIQUE INDEX {settings.statistics_table}_unique_count ON {settings.statistics_table} (priority, DATE_TRUNC('sec', created at time zone 'UTC'), status, entrypoint);""",
        ),
        Migration(
            version="014",
            description="Create pgqueuer_schedules table",
            sql_generator=lambda: f"""CREATE {durability.schedules_table} TABLE {settings.schedules_table} (id SERIAL PRIMARY KEY, expression TEXT NOT NULL, entrypoint TEXT NOT NULL, heartbeat TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL, created TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL, updated TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL, next_run TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL, last_run TIMESTAMP WITH TIME ZONE, status {settings.queue_status_type} DEFAULT 'queued', UNIQUE (expression, entrypoint));""",
        ),
        Migration(
            version="015",
    ...
    $$ LANGUAGE plpgsql;""",
        ...
        Migration(
            ...
            sql_generator=lambda: f"""CREATE TRIGGER {settings.trigger} AFTER INSERT OR UPDATE OR DELETE OR TRUNCATE ON {settings.queue_table} EXECUTE FUNCTION {settings.function}();""",
        ),
    ]
