...
def create_migrations_list() -> list[Migration]:
    ...
    migrations = [
        Migration(
            ...
            sql_generator=lambda: (
                f"CREATE TYPE {settings.queue_status_type} AS ENUM "
                f"('queued', 'picked', 'successful', 'exception', "
                f"'canceled', 'deleted');"
            ),
        ),
        Migration(
        ...
        Migration(
            ...
            sql_generator=lambda: (
                f"CREATE INDEX {settings.queue_table}_priority_id_id1_idx "
                f"ON {settings.queue_table} (priority ASC, id DESC) "
                f"INCLUDE (id) WHERE status = 'queued';"
            ),
        ),
        Migration(
            version="004",
            ...
            sql_generator=lambda: (
                f"CREATE INDEX {settings.queue_table}_updated_id_id1_idx "
                f"ON {settings.queue_table} (updated ASC, id DESC) "
                f"INCLUDE (id) WHERE status = 'picked';"
            ),
        ),
        Migration(
            version="005",
            ...
            sql_generator=lambda: (
                f"CREATE INDEX {settings.queue_table}_queue_manager_id_idx "
                f"ON {settings.queue_table} (queue_manager_id) "
                f"WHERE queue_manager_id IS NOT NULL;"
            ),
        ),
        Migration(
            version="006",
            ...
            sql_generator=lambda: (
                ...
                f"ON {settings.queue_table} (dedupe_key) "
                f"WHERE ((status IN ('queued', 'picked') "
                f"AND dedupe_key IS NOT NULL));"
            ),
        ),
        Migration(
            version="007",
            ...
            sql_generator=lambda: (
                f"CREATE {durability.queue_log_table} TABLE "
                f"{settings.queue_table_log} ("
                f"id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, "
                f"created TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL, "
                f"job_id BIGINT NOT NULL, "
                f"status {settings.queue_status_type} NOT NULL, "
                f"priority INT NOT NULL, "
                f"entrypoint TEXT NOT NULL, "
                f"traceback JSONB DEFAULT NULL, "
                f"aggregated BOOLEAN DEFAULT FALSE);"
            ),
        ),
        Migration(
            version="008",
            ...
            sql_generator=lambda: (
                f"CREATE INDEX {settings.queue_table_log}_not_aggregated "
                f"ON {settings.queue_table_log} ((1)) "
                f"WHERE not aggregated;"
            ),
        ),
        Migration(
            version="009",
            ...
            sql_generator=lambda: (
                f"CREATE INDEX {settings.queue_table_log}_created "
                f"ON {settings.queue_table_log} (created);"
            ),
        ),
        Migration(
            version="010",
            ...
            sql_generator=lambda: (
                f"CREATE INDEX {settings.queue_table_log}_status "
                f"ON {settings.queue_table_log} (status);"
            ),
        ),
        Migration(
            version="011",
            ...
            sql_generator=lambda: (
                f"CREATE INDEX {settings.queue_table_log}_job_id_status "
                f"ON {settings.queue_table_log} (job_id, created DESC);"
            ),
        ),
        Migration(
            version="012",
            ...
            sql_generator=lambda: (
                f"CREATE {durability.statistics_table} TABLE "
                f"{settings.statistics_table} ("
                f"id SERIAL PRIMARY KEY, "
                f"created TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT "
                f"DATE_TRUNC('sec', NOW() at time zone 'UTC'), "
                f"count BIGINT NOT NULL, "
                f"priority INT NOT NULL, "
                f"status {settings.queue_status_type} NOT NULL, "
                f"entrypoint TEXT NOT NULL);"
            ),
        ...
        Migration(
            ...
            sql_generator=lambda: (
                ...
                f"ON {settings.statistics_table} ("
                f"priority, "
                f"DATE_TRUNC('sec', created at time zone 'UTC'), "
                f"status, "
                f"entrypoint);"
            ),
        ),
        Migration(
            ...
            sql_generator=lambda: (
                f"CREATE {durability.schedules_table} TABLE "
                f"{settings.schedules_table} ("
                f"id SERIAL PRIMARY KEY, "
                f"expression TEXT NOT NULL, "
                f"entrypoint TEXT NOT NULL, "
                f"heartbeat TIMESTAMP WITH TIME ZONE DEFAULT NOW() "
                f"NOT NULL, "
                f"created TIMESTAMP WITH TIME ZONE DEFAULT NOW() "
                f"NOT NULL, "
                f"updated TIMESTAMP WITH TIME ZONE DEFAULT NOW() "
                f"NOT NULL, "
                f"next_run TIMESTAMP WITH TIME ZONE DEFAULT NOW() "
                f"NOT NULL, "
                f"last_run TIMESTAMP WITH TIME ZONE, "
                f"status {settings.queue_status_type} DEFAULT 'queued', "
                f"UNIQUE (expression, entrypoint));"
            ),
        ),
        Migration(
            version="015",
    ...
    $$ LANGUAGE plpgsql;""",
        ...
        Migration(
            ...
            sql_generator=lambda: (
                f"CREATE TRIGGER {settings.trigger} "
                f"AFTER INSERT OR UPDATE OR DELETE OR TRUNCATE "
                f"ON {settings.queue_table} "
                f"EXECUTE FUNCTION {settings.function}();"
            ),
        ),
    ]
