...
class QueryBuilderEnvironment:
    ...
    def build_install_query(self, create_schema: bool = True) -> str:
        ...
        qn = self.qualified
        create_schema_stmt = (
            f"CREATE SCHEMA IF NOT EXISTS {s.db_schema};\n    "
    ...
    CREATE INDEX {s.queue_table}_ep_prio_id_idx ON {qn.queue_table} (entrypoint, priority DESC, id ASC)
        WHERE status = 'queued';
    CREATE INDEX {s.queue_table}_ep_ea_idx ON {qn.queue_table} (entrypoint, execute_after)
        WHERE status = 'queued';
    CREATE UNIQUE INDEX IF NOT EXISTS {s.queue_table}_unique_dedupe_key ON
        {qn.queue_table} (dedupe_key) WHERE ((status IN ('queued', 'picked') AND dedupe_key IS NOT NULL));
    CREATE UNIQUE INDEX {s.queue_table}_picked_slot_idx ON {qn.queue_table} (entrypoint, slot)
        WHERE (status = 'picked' AND slot IS NOT NULL);

    CREATE {durability_policy.queue_log_table} TABLE {qn.queue_table_log} (
    ...
    CREATE {durability_policy.schedules_table} TABLE {qn.schedules_table} (
        id BIGSERIAL PRIMARY KEY,
    ...
    CREATE FUNCTION {qn.function}() RETURNS TRIGGER AS $$
    DECLARE
    ...
    def build_uninstall_query(self) -> str:
        s = self.settings
        qn = self.qualified
        return f"""DROP TRIGGER    IF EXISTS   {s.trigger} ON {qn.queue_table};
    DROP FUNCTION   IF EXISTS   {qn.function};
    DROP TABLE      IF EXISTS   {qn.queue_table};
    DROP TABLE      IF EXISTS   {qn.statistics_table};
    DROP TABLE      IF EXISTS   {qn.schedules_table};
    DROP TABLE      IF EXISTS   {qn.queue_table_log};
    DROP TYPE       IF EXISTS   {qn.queue_status_type};
    DROP TYPE       IF EXISTS   {s.qualify(s.legacy_statistics_status_type)};
    """  # noqa
    ...
    $$ LANGUAGE plpgsql;"""
        ...
        # Capacity slots enforcing concurrency_limit across MVCC snapshots
        # (issue #761): each picked job of a limited entrypoint holds one slot
        # in 0..limit-1, and the partial unique index rejects a second claim of
        # the same slot even when the first claim is invisible to the claimer's
        # snapshot. Rows picked before this migration (or by not-yet-upgraded
        # workers) have slot NULL and are only covered by the count-based gate;
        # they drain out as they complete.
...
class QueryQueueBuilder:
    ...
    def build_dequeue_query(
        ...
        *,
        ...
        if capacity_gated:
            # Unlimited entrypoints keep the direct locked scan: SKIP LOCKED may slide
            # arbitrarily far down the backlog, but with no limit to violate that is
            # pure throughput.
            unlimited_lookup = composer.nest(
                "CROSS JOIN LATERAL (",
            ...
            unlimited_pick = composer.clauses(
            ...
            # Limited entrypoints fix the candidate window before any locking: its
            # LIMIT caps the window at the remaining capacity and stops the planner
            # from pulling the subquery up, which would put the lock node back under
            # a LIMIT and restore the slide-down. Each window row is then re-fetched
            # by primary key in its own LATERAL and locked there, where no LIMIT
            # applies, so the quals are re-checked by EvalPlanQual against the newest
            # row version. SKIP LOCKED can then only skip rows inside the window: a
            # row locked by a concurrent worker shrinks this worker's pick instead of
            # exceeding the concurrency limit.
            capped_window = composer.nest(
                "FROM (",
                "SELECT candidate.id",
                f"FROM {queue_table} candidate",
                candidate_where,
                "ORDER BY candidate.priority DESC, candidate.id ASC",
                f"LIMIT LEAST({batch}, available.remaining)",
                footer=") capped",
            )
            locked_target_where = composer.where(
                "target.id = capped.id",
                "target.status = 'queued'",
                "target.execute_after < NOW()",
            )
            locked_target = composer.nest(
                "CROSS JOIN LATERAL (",
                "SELECT target.id, target.priority",
                f"FROM {queue_table} target",
                locked_target_where,
                "FOR UPDATE OF target SKIP LOCKED",
                footer=") locked",
            )
            limited_lookup = composer.nest(
                "CROSS JOIN LATERAL (",
            ...
            composer.cte(
                ...
                f"LIMIT {batch}",
                comment="New queued jobs, split by whether the entrypoint carries a limit.",
            )
        else:
        ...
        stale_columns = "SELECT stale.id, stale.priority"
        if capacity_gated:
            stale_columns = "SELECT stale.id, stale.priority, stale.entrypoint"
        composer.cte(
            "next_stale",
            stale_columns,
        ...
        if capacity_gated:
            # A stale re-pick keeps the slot it already holds, so only fresh claims
            # need one assigned below.
        ...
        if capacity_gated:
            # Every fresh claim on a limited entrypoint must take a slot in
            # 0..limit-1. The partial unique index on (entrypoint, slot) enforces the
            # limit ACROSS snapshots, because a btree uniqueness check sees
            # uncommitted tuples: two workers racing for the last slot both pass the
            # count gate, but the second one's insertion waits on the first and then
            # fails with 23505, aborting its dequeue for the caller to retry (#761).
            slot_holder_where = composer.where(
                "holder.entrypoint = params.entrypoint",
                "holder.status = 'picked'",
                "holder.slot = slots.slot",
            )
            slot_is_free = composer.nest(
                "NOT EXISTS (",
                f"SELECT FROM {queue_table} holder",
                slot_holder_where,
                footer=")",
            )
            slot_rank = "ROW_NUMBER() OVER (PARTITION BY params.entrypoint ORDER BY slots.slot)"
            free_slot_columns = composer.nest(
                "SELECT",
                "params.entrypoint,",
                "slots.slot,",
                f"{slot_rank} AS slot_rank",
            )
            free_slots_where = composer.where(
                "params.concurrency_limit > 0",
                slot_is_free,
            )
            generated_slots = "GENERATE_SERIES(0, params.concurrency_limit - 1) AS slots(slot)"
            composer.cte(
                "free_slots",
                free_slot_columns,
                "FROM params",
                f"CROSS JOIN LATERAL {generated_slots}",
                free_slots_where,
                comment="Capacity slots not held by a picked job, ranked per entrypoint.",
            )

            fresh_rank = (
                "ROW_NUMBER() OVER (PARTITION BY entrypoint ORDER BY priority DESC, id ASC)"
            )
            ranked_fresh_columns = composer.nest(
                "SELECT",
                "id,",
                "entrypoint,",
                f"{fresh_rank} AS slot_rank",
            )
            ranked_fresh = composer.nest(
                "FROM (",
                ranked_fresh_columns,
                "FROM eligible",
                "WHERE fresh",
                footer=") ranked",
            )
            free_slot_join = composer.nest(
                "JOIN free_slots",
