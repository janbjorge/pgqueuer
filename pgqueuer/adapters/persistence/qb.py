...
class QueryBuilderEnvironment:
    ...
    $$ LANGUAGE plpgsql;"""
        ...
        yield f"ALTER TABLE {self.settings.statistics_table} ALTER COLUMN status TYPE {self.settings.queue_status_type} USING status::TEXT::{self.settings.queue_status_type};"  # noqa
        ...
        # Modernize legacy int4 SERIAL id columns on pre-existing installs (#671).
        # int4 SERIAL caps lifetime ids at ~2.1B; once exceeded inserts fail. Two
        # steps per table: widen the column int4 -> bigint, then swap the owned
        # SERIAL sequence for a GENERATED BY DEFAULT AS IDENTITY sequence so the
        # upgraded shape matches a fresh install exactly. Both take an ACCESS
        # EXCLUSIVE lock (the widen also rewrites the table), so they block
        # briefly. Gated by settings.widen_id so operators can skip it and migrate
        # a large table out-of-band; the DO-guards keep re-runs a no-op.
        if self.settings.widen_id:
            for table in (
                ...
                yield self.build_convert_id_to_identity_query(table)
    ...
    def build_convert_id_to_identity_query(self, table: str) -> str:
        """Convert a legacy SERIAL/BIGSERIAL id into GENERATED BY DEFAULT AS IDENTITY.

        Widening the column alone leaves the owned SERIAL sequence in place, so
        upgraded installs diverged from fresh ones (which now use IDENTITY). This
        drops the SERIAL default + sequence and attaches an identity sequence,
        restarting it past both MAX(id) and the old sequence's next value so no id
        is ever reused. Guarded on attidentity: '' means "not an identity" (the
        type is "char"; '' is the zero byte), so re-runs and fresh installs skip.
        """
        return f"""DO $$
    DECLARE
        seq TEXT;
    ...
    BEGIN
        IF EXISTS (
            ...
            END IF;
            restart_at := GREATEST(max_id + 1, COALESCE(seq_next, 1));
            ...
            EXECUTE format('ALTER TABLE {table} ALTER COLUMN id RESTART WITH %s', restart_at);
        END IF;
    END $$;"""
