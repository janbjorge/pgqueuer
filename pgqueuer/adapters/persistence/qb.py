...
class QueryBuilderEnvironment:
    ...
    def build_upgrade_queries(self) -> Generator[str, None, None]:
        ...
        yield f"CREATE INDEX IF NOT EXISTS {s.queue_table}_priority_id_id1_idx ON {qn.queue_table} (priority ASC, id DESC) INCLUDE (id) WHERE status = 'queued';"  # noqa: E501
    ...
    $$ LANGUAGE plpgsql;"""
        ...
        yield f"""CREATE {durability_policy.statistics_table} TABLE IF NOT EXISTS {qn.statistics_table} (
        id BIGSERIAL PRIMARY KEY,
        created TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT DATE_TRUNC('sec', NOW() at time zone 'UTC'),
        count BIGINT NOT NULL,
        priority INT NOT NULL,
        ...
        yield f"""CREATE UNIQUE INDEX IF NOT EXISTS {s.statistics_table}_unique_count ON {qn.statistics_table} (
        priority,
        DATE_TRUNC('sec', created at time zone 'UTC'),
        status,
        entrypoint
        ...
        # also what heals a pre-1.4 install that needed no DDL at all.
        yield self.build_schema_marker_upgrade_statement()
    ...
    def build_schema_marker_upgrade_statement(self) -> str:
        """The marker batch for `upgrade`, tolerant of objects it cannot create.

        `upgrade` runs against schemas it did not build; a COMMENT on an absent
        object would abort the run and leave the operator no way forward.
        """
        statements = ",\n            ".join(
            f"$marker${statement}$marker$" for statement in self.build_schema_marker_statements()
        )
        return f"""DO $$
    DECLARE
        statement TEXT;
    ...
    def build_schema_markers_query(self) -> str:
        ...
        """
        # Relkinds the pre-marker information_schema probes accepted: narrowing
        # here would call a working view-backed install "not installed".
        relations = "('r', 'p', 'v', 'm', 'f')"
        if self.settings.db_schema:
            namespace = (
                f"(SELECT oid FROM pg_namespace WHERE nspname = {self.settings.schema_expr})"
            )
            relation_scope = f"relation.relnamespace = {namespace}"
            type_scope = f"enum_type.typnamespace = {namespace}"
            routine_scope = f"routine.pronamespace = {namespace}"
        else:
            # search_path installs resolve by visibility, not by current_schema().
            relation_scope = "pg_table_is_visible(relation.oid)"
            type_scope = "pg_type_is_visible(enum_type.oid)"
            routine_scope = "pg_function_is_visible(routine.oid)"

        return f"""SELECT 'table' AS kind, relation.relname AS name, NULL::name AS parent,
               obj_description(relation.oid, 'pg_class') AS description
        FROM pg_class relation
        WHERE {relation_scope}
          AND relation.relkind IN {relations} AND relation.relname = ANY ($1)
        UNION ALL
        ...
        WHERE {relation_scope}
          AND relation.relkind IN ('i', 'I') AND relation.relname = ANY ($2)
        UNION ALL
        SELECT 'type', enum_type.typname, NULL, obj_description(enum_type.oid, 'pg_type')
        FROM pg_type enum_type
        WHERE {type_scope} AND enum_type.typname = ANY ($3)
        UNION ALL
        SELECT 'enum_value', enum_label.enumlabel, enum_type.typname, NULL
        FROM pg_enum enum_label
        JOIN pg_type enum_type ON enum_type.oid = enum_label.enumtypid
        WHERE {type_scope} AND enum_type.typname = ANY ($3)
        UNION ALL
        SELECT 'function', routine.proname, NULL, obj_description(routine.oid, 'pg_proc')
        FROM pg_proc routine
        WHERE {routine_scope} AND routine.proname = ANY ($4)
        UNION ALL
        SELECT 'trigger', trigger_row.tgname, relation.relname,
               obj_description(trigger_row.oid, 'pg_trigger')
        FROM pg_trigger trigger_row
        JOIN pg_class relation ON relation.oid = trigger_row.tgrelid
        WHERE {relation_scope}
          AND NOT trigger_row.tgisinternal AND relation.relname = ANY ($1);"""
