...
- **`id` columns widened to `BIGINT` identity — no more ~2.1B lifetime-enqueue
  ceiling ([#671](https://github.com/janbjorge/pgqueuer/issues/671)):** The
  `queue`, `statistics`, and `schedules` primary keys were `int4 SERIAL`, capped
  at 2,147,483,647. Because the sequence only climbs and never reuses values, a
  long-lived deployment would eventually exhaust it and every `enqueue` would
  fail. Fresh installs now use `BIGINT GENERATED BY DEFAULT AS IDENTITY`; `pgq
  upgrade` widens existing `int4` columns to `BIGINT` and swaps their legacy
  `SERIAL` sequences (created `AS INTEGER`, so they would otherwise still cap at
  2^31-1) for identity sequences, so an upgraded schema matches a fresh install.
  The conversion preserves the id number line — the identity restarts past the
  old sequence and never reuses an id.
  ...
  > migration is idempotent (guarded on the column type and identity attribute),
  > so it is a no-op once everything is `BIGINT` identity and safe to re-run.
...
1. **Schema:** Run `pgq install` or `pgq upgrade`. Both add the `attempts`
   ...
   widens the `int4` `id` columns to `BIGINT` and converts them to identity
   (#671) — this takes an `ACCESS EXCLUSIVE` lock and rewrites each table, so run
   it in a maintenance window on large tables (or `pgq upgrade --no-widen-id` and
   migrate out-of-band).
