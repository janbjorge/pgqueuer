...
- **`id` columns widened to `BIGINT` — no more ~2.1B lifetime-enqueue ceiling
  ...
  fail. Fresh installs now use `BIGINT GENERATED BY DEFAULT AS IDENTITY`
  (matching `queue_log`); `pgq upgrade` widens existing `int4` columns in place.
  ...
  > during a maintenance window or low-traffic period.** If you need zero
  > downtime on a very large table, perform the widen manually with an online
  > strategy (new `BIGINT` column + batched backfill + swap) instead of
  > `pgq upgrade`.
  >
  > The migration is idempotent — it checks the current column type and is a
  > no-op once the column is already `BIGINT`, so it is safe to re-run.
