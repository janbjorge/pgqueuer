...
async def test_job_queuing(
    ...
    q = Queries(apgdriver)
    c = QueueManager(connection=apgdriver, queries=q, resources={"test": "job_queuing"})
    seen = list[int]()
...
async def test_job_fetch(
    ...
    qmpool = [
        QueueManager(
            connection=apgdriver, queries=Queries(apgdriver), resources={"test": "job_fetch"},
        )
...
async def test_pick_local_entrypoints(
    ...
    q = Queries(apgdriver)
    qm = QueueManager(
        connection=apgdriver, queries=Queries(apgdriver), resources={"test": "pick_local"},
    )
    pikced_by = list[str]()
...
async def test_pick_set_queue_manager_id(
    ...
    q = Queries(apgdriver)
    qm = QueueManager(
        connection=apgdriver, queries=Queries(apgdriver), resources={"test": "pick_qm_id"},
    )
    qmids = set[uuid.UUID]()
