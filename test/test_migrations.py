...
from pgqueuer import db, migrations, qb, queries
...
async def test_create_migrations_list() -> None:
    ...
    default_settings = qb.DBSettings()
    migration_list = list(migrations.create_migrations_list(default_settings))
...
async def test_create_migrations_list_respects_durability() -> None:
    """Migrations should honor provided durability settings."""
    volatile_settings = qb.DBSettings(durability=qb.Durability.volatile)
    migration_list = list(migrations.create_migrations_list(volatile_settings))

    queue_table_migration = next(m for m in migration_list if m.version == "002")
    sql = queue_table_migration.sql_generator()

    assert "CREATE UNLOGGED TABLE" in sql


async def test_create_migrations_list_respects_overrides() -> None:
    """Migrations should use explicit DBSettings overrides."""
    settings = qb.DBSettings(queue_table="pref_pgqueuer")

    migration_list = list(migrations.create_migrations_list(settings))
    queue_table_migration = next(m for m in migration_list if m.version == "002")
    sql = queue_table_migration.sql_generator()

    assert settings.queue_table in sql


async def test_migration_versions_sequential() -> None:
    """Test that migration versions are sequential with no gaps."""
    migration_list = migrations.create_migrations_list(qb.DBSettings())
