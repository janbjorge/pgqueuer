...
async def test_execute_after_default_is_now(apgdriver: Driver) -> None:
    await Queries(apgdriver).qq.enqueue("foo", None, 0, None)
    assert (
        len(
    ...
    await Queries(apgdriver).qq.enqueue("foo", None, 0)
    assert (
        len(
            await Queries(apgdriver).qq.dequeue(
                10,
...
async def test_execute_after_zero(apgdriver: Driver) -> None:
    ...
    await Queries(apgdriver).qq.enqueue("foo", None, 0, execute_after)
    assert (
        len(
            await Queries(apgdriver).qq.dequeue(
                10,
...
async def test_execute_after_1_second(apgdriver: Driver) -> None:
    ...
    await Queries(apgdriver).qq.enqueue("foo", None, 0, execute_after)
    before = await Queries(apgdriver).qq.dequeue(
        10,
        {"foo": EntrypointExecutionParameter(timedelta(seconds=60), False, 0)},
        uuid.uuid4(),
    )
    ...
    after = await Queries(apgdriver).qq.dequeue(
        10,
        {"foo": EntrypointExecutionParameter(timedelta(seconds=60), False, 0)},
        uuid.uuid4(),
    )
