...
async def id_is_identity(driver: db.Driver, table: str) -> bool:
    """True if ``table.id`` is a GENERATED ... AS IDENTITY column.

    ``pg_attribute.attidentity`` is ``'a'`` (ALWAYS) or ``'d'`` (BY DEFAULT) for
    an identity column, and the zero byte for a plain SERIAL/BIGSERIAL column.
    Comparing in SQL sidesteps the driver's bytes-vs-str encoding of ``"char"``.
    """
    rows = await driver.fetch(
        """SELECT attidentity IN ('a', 'd') AS is_identity FROM pg_attribute
        WHERE attrelid = $1::regclass
          AND attname = 'id'
          AND NOT attisdropped;""",
        table,
    )
    return bool(rows[0]["is_identity"])


async def id_has_serial_sequence(driver: db.Driver, table: str) -> bool:
    """True if a plain SERIAL ('a' dep) sequence is still attached to ``table.id``.

    An identity column's sequence is an internal ('i') dependency, so a correct
    conversion leaves no SERIAL-owned sequence behind.
    """
    rows = await driver.fetch(
        """SELECT EXISTS (
    ...
    return bool(rows[0]["has_serial_seq"])
