...
async def id_is_identity(driver: db.Driver, table: str) -> bool:
    ...
    ``pg_attribute.attidentity`` is ``''`` (the zero byte) for a plain
    SERIAL/BIGSERIAL column and ``'a'``/``'d'`` for an identity column.
    """
    rows = await driver.fetch(
    ...
    raw = rows[0]["attidentity"]
    value = raw.decode() if isinstance(raw, bytes) else raw
    # 'a' = GENERATED ALWAYS, 'd' = GENERATED BY DEFAULT; anything else
    # (the zero byte, surfaced as '' or '\x00') is a plain SERIAL column.
    return value in ("a", "d")


async def id_serial_sequence_count(driver: db.Driver, table: str) -> int:
    """Number of sequences attached to ``table.id`` via a plain SERIAL ('a') dep.

    An identity column's sequence is an internal ('i') dependency, so a correct
    conversion leaves zero SERIAL-owned sequences behind.
    """
    rows = await driver.fetch(
